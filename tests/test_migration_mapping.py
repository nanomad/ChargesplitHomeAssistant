"""Unit tests for migrate_unique_id (pure function)."""
...
# Derived from the v0.0.7 contract in tests/test_setup.py::EXPECTED_V007.
SENSOR_CASES = [
...
def test_serial_with_dash_is_parsed_safely():
    serial = "ABC-123"
    assert (
        migrate_unique_id(f"Chargesplit-{serial}-Voltage L1-{serial}", serial)
